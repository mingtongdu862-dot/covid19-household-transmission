"""
XGBoost – Individual-Level Susceptibility Prediction
=====================================================
Task   : Predict whether a household-exposed individual will be infected
         (secondary case) given their demographic, socioeconomic, and
         medical-history features.
Label  : label  (1 = secondary case / infected, 0 = healthy / not infected)
Dataset: Personal_Level_Dataset_V2/

Design note on the label
------------------------
Co-primary cases (original label=1) are excluded from this dataset.
Only secondary cases (original label=2 → relabelled to 1) and healthy
household members (original label=0) are retained, and only from households
where transmission did occur (T_h=1).  This isolates individual
susceptibility conditional on household exposure.
...
3. All naming, labels, and output files are specific to the individual task.
"""
...
# ==================== CONFIGURATION ====================
TASK_NAME  = 'Individual Susceptibility Prediction'
output_dir = 'XGB_results_Individual/'
os.makedirs(output_dir, exist_ok=True)

# Columns to drop before training (non-feature identifiers / leakage risks)
...
def load_and_preprocess(fold: int):
    base_path = 'Individual_Level_Dataset_FrameworkII'
    drop_cols = ['person_id'] + deleted_cols   # IndexDate already in deleted_cols
    label_col = 'label'
...
def print_feature_distribution(X_train: np.ndarray,
    ...
    Shows mean, std, median, min, max, and % nonzero per feature, which helps
    identify whether a high-importance feature is driven by a few extreme cases
    (check std/max) or is a sparse binary feature (check % > 0).
    """
    ...
    for rank, idx in enumerate(ranked, start=1):
        col      = X_train[:, idx]
        col_v    = col[~np.isnan(col)]
        mean_v   = np.mean(col_v)   if len(col_v) else np.nan
        std_v    = np.std(col_v)    if len(col_v) else np.nan
        med_v    = np.median(col_v) if len(col_v) else np.nan
        min_v    = np.min(col_v)    if len(col_v) else np.nan
        max_v    = np.max(col_v)    if len(col_v) else np.nan
        pct_nz   = (col_v > 0).mean() * 100 if len(col_v) else np.nan
...
print(f"Positive class     : secondary cases (infected via household transmission)")
print(f"Negative class     : healthy household members (exposed, not infected)")
...
for fold in range(1, 6):
    ...
    for split_name, metrics in [('VALIDATION', val_metrics), ('TEST', test_metrics)]:
        ...
        print(f"\n  Class 1 (Susceptible individual — secondary case):")
...
summary_path    = os.path.join(output_dir, 'individual_xgboost_summary.csv')
importance_path = os.path.join(output_dir, f'individual_xgboost_top{top_n}_features.csv')
full_json_path  = os.path.join(output_dir, 'individual_xgboost_full_results.json')
